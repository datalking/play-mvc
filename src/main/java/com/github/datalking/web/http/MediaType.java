...
public class MediaType implements Comparable<MediaType> {
    ...
    public MediaType(String type, String subtype, Map<String, String> parameters) {
        ...
        Assert.notNull(subtype, "subtype must not be empty");
        checkToken(type);
        checkToken(subtype);
    ...
    public static MediaType parseMediaType(String mediaType) {
        ...
        if (subIndex == -1) {
            try {
                throw new Exception(mediaType + "does not contain '/'");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        if (subIndex == fullType.length() - 1) {
            try {
                throw new Exception(mediaType + "does not contain subtype after '/'");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        if (WILDCARD_TYPE.equals(type) && !WILDCARD_TYPE.equals(subtype)) {
            try {
                throw new Exception(mediaType + "wildcard type is legal only in '*/*' (all media types)");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        try {
            ...
            try {
                throw new Exception(mediaType + "unsupported charset '" + ex.getCharsetName() + "'");
            } catch (Exception e) {
                e.printStackTrace();
            }
        } catch (IllegalArgumentException ex) {
            try {
                throw new Exception(mediaType + ex.getMessage());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
